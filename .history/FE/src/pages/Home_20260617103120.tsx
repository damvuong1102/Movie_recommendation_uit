    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-2 pb-8 flex flex-col gap-3">
        ...
        <div className="space-y-10 mt-2">
          ...
          {isSearching && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 border-b pb-2">
                ...
                <h2 className="text-xl font-bold">Kết quả tìm kiếm cho "{debouncedSearch}"</h2>
              </div>

              {sections.searchResults.loading ? (
                <div className="text-center py-12 text-muted-foreground">Đang tìm kiếm...</div>
              ) : sections.searchResults.movies.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">Không tìm thấy phim nào phù hợp với tìm kiếm của bạn.</div>
              ) : (
          ...
          {/* DANH MỤC: DÀNH CHO BẠN (RECOMMENDED) */}
          {isAuthenticated && showRecommended && (
            <MovieSection
              title="Dành Cho Bạn"
              icon={ThumbsUp}
              ...
              loadingMore={sections.recommended.loadingMore}
              onLoadMore={() => loadMore("recommended")}
              /* Ép buộc phần này chỉ dùng MovieCard bình thường (1 sao vàng kèm điểm số) */
              renderCard={(movie) => (
                <MovieCard key={movie.id} {...movie} />
              )}
            />
          ...
          {((category === "recentlyWatched" && recentlyWatched.length === 0) || (category === "recentlyWatched" && !isAuthenticated)) && !isSearching && (
            <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
              ...
              <h3 className="text-lg font-medium">Chưa có phim nào được xem gần đây</h3>
            </div>
          )}
        </div>
      </main>
