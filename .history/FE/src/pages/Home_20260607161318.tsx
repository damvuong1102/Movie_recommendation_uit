...
//   // FIX: fetchSection đọc genre/search từ ref → không bao giờ bị stale closure
...
//           type,
//           page,
//           size: key === "searchResults" ? 25 : 10,
//           // FIX: đọc từ ref thay vì closure — luôn là giá trị hiện tại
...
//   // FIX: theo dõi debouncedSearch (không phải search thô) → không có race condition
//   useEffect(() => {
...
//   const showRecentlyWatched = !isSearching && (showAll || category === "recentlyWatched");
...
//           {category === "recentlyWatched" && recentlyWatched.length === 0 && !isSearching && (
...
export default function Home() {
  ...
  const showTrending          = !isSearching && (showAll || category === "trending");
  const showRecommended       = !isSearching && (showAll || category === "recommended");
  // FIX: Chỉ kích hoạt điều kiện hiển thị Recently Watched khi đã đăng nhập hệ thống
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-2 pb-8 flex flex-col gap-3">
        ...
        {!isSearching && setCategory && category !== undefined && (
          <div className="flex items-center gap-1 overflow-x-auto scrollbar-none pb-0.5">
            {CATEGORIES
              // FIX: Lọc bỏ mục 'Recently Watched' và 'Recommended' trên thanh filter nếu chưa đăng nhập
